...
class Num2Word_ptBR(Num2Word_EU):
    ...
    def __init__(self):
        ...
        self.errmsg_nonnum = \
            "Apenas números podem ser convertidos em palavras."

        self.errmsg_toobig = \
            "Número muito grande para ser convertido em palavras."
        ...
        self.mid_numwords = [
            (1000, "mil"), (100, "cem"), (90, "noventa"), (80, "oitenta"),
    ...
    def merge(self, curr, next):
        ...
        if nnum < cnum:
            if cnum > 100 and nnum > 100 and nnum % 100:
    ...
    def to_ordinal(self, value):
        ...
        try:
            if value == 0:
                ...
                text = "%s%s%s" % (
                self.ords[10], self.gender_stem, self.to_ordinal(value - 10))
            elif value <= 100:
                dec = (value / 10) * 10
                text = "%s%s %s" % (
                self.ords[dec], self.gender_stem, self.to_ordinal(value - dec))
            elif value <= 1e3:
                cen = (value / 100) * 100
                text = "%s%s %s" % (
                self.ords[cen], self.gender_stem, self.to_ordinal(value - cen))
            elif value < 1e18:
                ...
                cardinal = self.to_cardinal(
                    part / dec) if part / dec != 1 else ""
                text = "%s%s%s %s" % (
                cardinal, self.ords[dec], self.gender_stem,
    ...
    def to_currency(self, val, longval=True, old=False):
        if old:
            return self.to_splitnum(val, hightxt="real/is", lowtxt="centavo/s",
                                    divisor=1000, jointxt="e", longval=longval)
        return super(Num2Word_ptBR, self).to_currency(val, jointxt="e",
                                                      longval=longval)
